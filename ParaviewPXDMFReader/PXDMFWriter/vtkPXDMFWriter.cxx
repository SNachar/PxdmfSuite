...
{
  public:
    vtkPxdmfWriterDomainMemoryHandler()
      {
        domain = new xdmf2::XdmfDomain();
      }
    ~vtkPxdmfWriterDomainMemoryHandler()
      {
    ...
    void InsertIntoRoot(xdmf2::XdmfRoot& root)
      {
        root.Insert(domain);
      }
  private:
    xdmf2::XdmfDomain* domain;
...
vtkStandardNewMacro(vtkPXDMFWriter);

//----------------------------------------------------------------------------
vtkPXDMFWriter::vtkPXDMFWriter()
{
  this->SetASCII(true);
}
...
//----------------------------------------------------------------------------
void vtkPXDMFWriter::PrintSelf(ostream& os, vtkIndent indent)
{
...
int vtkPXDMFWriter::Write()
{
  //std::cout << this->GetFileName() << " ---- " << std::endl;
  //std::cout << this->GetHeavyDataFileName() << " heavy " << std::endl;
  internal_cpt = 1;
...
//
void vtkPXDMFWriter::SetASCII(bool ascii){
  if(ascii){
...
//
void vtkPXDMFWriter::SetFileName (const char* _arg) {
  vtkXdmfWriter::SetFileName(_arg);
  
  std::string heavyfile = this->GetFileName() ;
  heavyfile = heavyfile.substr(0, heavyfile.find_last_of(".")) + ".h5";
  //std::cout << heavyfile << " heavy " << std::endl;
...
//----------------------------------------------------------------------------
/*
int vtkPXDMFWriter::RequestData(
...
int vtkPXDMFWriter::WriteCompositeDataSet(vtkCompositeDataSet *dobj, xdmf2::XdmfGrid *grid)
{
  ...
  //the first  grid we use the grid from up
  
  xdmf2::XdmfGrid *childsGrid = grid;
  //childsGrid->SetDeleteOnGridDelete(true);
  vtkDataObject* ds = iter->GetCurrentDataObject();
  this->WriteDataSet(ds, grid);
  iter->GoToNextItem();
  
  while (!iter->IsDoneWithTraversal()){
    
    childsGrid = new xdmf2::XdmfGrid();
    childsGrid->SetDeleteOnGridDelete(true);

        
    ((vtkPxdmfWriterDomainMemoryHandler*) this->DomainMemoryHandler)->InsertGrid(childsGrid);
    childsGrid->SetDOM(grid->GetDOM());
...
int vtkPXDMFWriter::CreateTopology(vtkDataSet *ds, xdmf2::XdmfGrid *grid, vtkIdType PDims[3], vtkIdType CDims[3], vtkIdType &PRank, vtkIdType &CRank, void *staticdata)
{
  ...
  xdmf2::XdmfTopology *t = grid->GetTopology();
  ...
  case VTK_UNIFORM_GRID:
    ...
    xdmf2::XdmfDataDesc *dd = t->GetShapeDesc();
    dd->SetShape(3, Dims);
  ...
  case VTK_UNSTRUCTURED_GRID:
    ...
    if ( cellTypes.size() == 1 )
      ...
      xdmf2::XdmfArray *di = t->GetConnectivity();
      ...
      for (vtkIdType cid=0 ; cid < numCells; cid++)
        ...
        switch(cellType)
          ...
          break;
          case VTK_QUADRATIC_EDGE :
            da->InsertValue(cntr++, XDMF_EDGE_3);
          break;
          default :
...
int vtkPXDMFWriter::WriteAtomicDataSet(vtkDataObject *dobj, xdmf2::XdmfGrid *grid)
{
  //cerr << "Writing " << dobj << " a " << dobj->GetClassName() << endl;
  ...
  /// we are going to print a actual dim so we increment the counter
  ...
  grid->SetName(("PGD"+to_string(internal_cpt)).c_str());
  this->SetHeavyDataGroupName(("PGD"+to_string(internal_cpt)).c_str());
  ++internal_cpt;
  ...
  vtkFieldData* inFD = dobj->GetFieldData();
  int nbdims = 2;
  for(; nbdims >= 0 ; --nbdims){
  ...
  xdmf2::XdmfInformation* in = new xdmf2::XdmfInformation();
  in->SetName("Dims");
  in->SetValue(to_string(nbdims).c_str());
  grid->Insert(in);
  for(int i = 0; i < nbdims ; i++){
    xdmf2::XdmfInformation* name = new xdmf2::XdmfInformation();
    name->SetDeleteOnGridDelete(true);  
    
    xdmf2::XdmfInformation* unit = new xdmf2::XdmfInformation();
    unit->SetDeleteOnGridDelete(true);
    
    name->SetName(("Dim"+to_string(i)).c_str());
    unit->SetName(("Unit"+to_string(i)).c_str());  
  ...
  grid->GetTopology()->SetLightDataLimit(this->LightDataLimit);
